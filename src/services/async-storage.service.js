...
function query(entityType, filterBy, delay = 500) {
  let entities =
    JSON.parse(localStorage.getItem(entityType)) || _createDogs();
...
function get(entityType, entityId) {
  return query(entityType).then((entities) => {
    const dog = entities.find((entity) => entity._id === entityId);
    return dog;
  });
}
...
function _createDogs() {
  const dogs = [
    {
      ...
      title: "Sprint 4",
      description:
        "This dog is for managing a single project. You can customize this dog to suit your project needs: add columns, subtasks, automations, dashdogs and more!",
  ...
  localStorage.setItem("dogDB", JSON.stringify(dogs));
  return dogs;
}
